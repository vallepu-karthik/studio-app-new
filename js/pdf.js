...
/* ══════════════════════════════════════════════════════
   TOTALS BLOCK  (subtotal / discount / GST / grand total)
══════════════════════════════════════════════════════ */
function drawTotalsBlock(doc, subtotal, discount, gst, gstRate, grandTotal, y) {
  var lx = ML + 95;
  var rx = PW - MR;
  ...
  if (discount > 0) {
    doc.setFontSize(10); st(doc, [29, 111, 66]); // green
  ...
  if (gst > 0) {
    st(doc, MID);
    doc.text('GST (' + gstRate + '%)', lx, y); right(doc, fmtINR(gst), rx, y); y += 7;
  }
...
function generateQuotePDF(quote) {
  ...
  var discount   = quote.discount  || 0;
  var gst        = quote.gst       || 0;
  var gstRate    = quote.gstRate   || 0;
  var grandTotal = quote.total     || 0;

  y = drawTotalsBlock(doc, subtotal, discount, gst, gstRate, grandTotal, y);
  y += 4;
